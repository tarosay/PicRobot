...
DWORD _USB_waitUntilCompleteTransaction(BYTE addr,BYTE ep,BYTE* ercode);
boolean device_attached = false;

// dummy function
// to use arduino-based software on pic
// yishii
							   ...
							   //adb_eventHandler * handler)
{
    ...
    //yishii connection->eventHandler = handler;
 ...
 */
boolean ADB_isAdbInterface(usb_interfaceDescriptor * interface)
{
 ...
 */
boolean ADB_isAdbDevice(/*usb_device * device, */int configuration, adb_usbConfiguration * handle)
{
	boolean ret = false;
	uint8_t buf[MAX_BUF_SIZE];
	int bytesRead;
 ...
 */
void ADB::(usb_device * device, adb_usbConfiguration * handle)
{
...
BOOL USB_ApplicationEventHandler ( BYTE address, USB_EVENT event, void *data, DWORD size )
{
	...
	switch (event){
		case EVENT_GENERIC_ATTACH:
			tprintf("%s : EVENT_GENERIC_ATTACH\r\n",__func__);
...
/*
  Wait until the requested transaction is finished
  Yasuhiro ISHII
*/
DWORD _USB_waitUntilCompleteTransaction(BYTE addr,BYTE ep,BYTE* ercode)
{
...
//	tprintf("[%s] wait for complete...",__func__);
...
//	tprintf("done\r\n");
	...
	if(local_errcode != USB_SUCCESS){
		if (local_errcode == USB_ENDPOINT_STALLED){
			...
			tprintf("*** Error occurred,but cannot addr=%d,EP=%02Xh,resolve code=%02Xh\r\n",
				addr,
				ep,
				local_errcode);
		}
